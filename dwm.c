...
void applyrules(Client *c) {
  ...
  if (ch.res_name)
    XFree(ch.res_name);
  /* Center floating windows if no floatx/floaty was set */
  if (c->isfloating && (c->x == 0 && c->y == 0)) {
...
void drawbar(Monitor *m) {
  ...
  for (i = 0; i < LENGTH(tags); i++) {
    ...
    int textw = TEXTW(tags[i]) - lrpad;
    int padding = (w - textw) / 2;
    drw_text(drw, x, 0, w, bh, padding, tags[i], urg & 1 << i);
    x += w;
  }
  drw_setscheme(drw, scheme[SchemeNorm]);
  x = drw_text(drw, x, 0, 0, bh, lrpad / 2, m->ltsymbol, 0);
